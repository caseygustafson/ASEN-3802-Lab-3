function [x_b,y_b] = NACA_Airfoils(m,p,t,c,N, x, order)
...
y_c1 = @(x) m*(x/(p^2))*(2*p-(x/c)); 

% Zeta function from 0 < x < p*c
...
% second part of peicewise function from pc < x < c
y_c2 = @(x) m*((c-x)/((1-p)^2)) * (1+(x/c)-2*p); 

% Zeta function from pc < x < c
