...
for i = 1:length(AR_list)
    ...
    for j = 1:length(taper)
        ...
        [e, ~, ~] = PLLT(b_fig, a0_t_fig, a0_r_fig, c_t_fig, c_r_fig, ...
                         aero_t_fig, aero_r_fig, geo_t_fig, geo_r_fig, N_fig);
...
% Wing Geo in feet
b    = 33 + 4/12;   c_r = 5 + 4/12;   c_t = 3 + 8.5/12;
AR   = (2*b) / (c_r + c_t);
S    = (b/2) * (c_r + c_t);            % wing area [ft^2]

% Get lift slope and zero-lift AoA from VPM
[~,~,XB_r,YB_r,aero_r] = NACA_Airfoils(2412, N_min_actual);
[~,~,XB_t,YB_t,aero_t] = NACA_Airfoils(  12, N_min_actual);
a0_r = (Vortex_Panel(XB_r,YB_r,5) - Vortex_Panel(XB_r,YB_r,0)) / deg2rad(5); % [/rad]
...
%% Deliverables 1 & 2: Convergence Study at alpha = 4 deg
N_vals = 1:2:201;                       % odd terms only
gR = deg2rad(4);  gT = deg2rad(4);      % 4 deg AoA, no twist for convergence

CL_c = zeros(1,length(N_vals));  CDi_c = zeros(1,length(N_vals));
for i = 1:length(N_vals)
    [~, CL_c(i), CDi_c(i)] = PLLT(b,a0_t,a0_r,c_t,c_r,aero_t,aero_r,gT,gR,N_vals(i));
end

% Relative errors vs highest-N reference
err_CL  = abs((CL_c  - CL_c(end))  / CL_c(end))  * 100;
err_CDi = abs((CDi_c - CDi_c(end)) / CDi_c(end)) * 100;

% Find first N within each threshold
thresholds = [10, 1, 0.1];
N_CL = zeros(1,3); N_CDi = zeros(1,3); CL_t = zeros(1,3); CDi_t = zeros(1,3);
for t = 1:3
...
% Deliverable 1 table
fprintf('\nCONVERGENCE TABLE\n');
fprintf('%-12s | %-8s | %-10s | %-8s | %-10s\n','Err (%)','N(CL)','CL','N(CDi)','CDi');
fprintf('%s\n', repmat('-',1,55));
for t = 1:3
    fprintf('%-12.1f | %-8d | %-10.6f | %-8d | %-10.6f\n', ...
        thresholds(t), N_CL(t), CL_t(t), N_CDi(t), CDi_t(t));
end
...
for sp = 1:2
    subplot(2,1,sp);
    if sp==1; err=err_CL;  lbl='C_L';    else; err=err_CDi; lbl='C_{D,i}'; end
    semilogy(N_vals, err, 'k-', 'LineWidth',1.8); hold on; grid on;
    for t=1:3
        N_mark = [N_CL(t), N_CDi(t)]; xline(N_mark(sp), '--', 'Color', colors{t}, ...
            'LineWidth',1.5, 'Label', sprintf('%.1f%%',thresholds(t)));
    end
    xlabel('Odd Terms N'); ylabel('Relative Error [%]');
    title([lbl ' Convergence']); ylim([1e-4 200]);
end
sgtitle('Convergence Study (\alpha = 4°)');

%% Deliverable 3: Cruise Performance (100 kts, 10,000 ft) 
N_use    = max(N_CL(3), N_CDi(3));      % most restrictive N for 0.1% error
rho      = 0.001756;                    % density at 10,000 ft [slug/ft^3]
V        = 100 * 1.68781;               % 100 knots -> ft/s
q        = 0.5 * rho * V^2;            % dynamic pressure [lb/ft^2]

[e_cr, CL_cr, CDi_cr] = PLLT(b,a0_t,a0_r,c_t,c_r,aero_t,aero_r,gT,gR,N_use);

%% ADJUST THIS SECTION I ESTIMATED THE 0.0065 AND 0.0060 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
cd_profile = 0.5*(0.0065 + 0.0060);    % avg of root (2412) and tip (0012)
CD_cr = cd_profile + CDi_cr;
L  = q*S*CL_cr;   Di = q*S*CDi_cr;   D = q*S*CD_cr;

fprintf('\nCRUISE (N=%d)\n', N_use);
fprintf('  CL=%.4f  CDi=%.4f  cd=%.4f  CD=%.4f\n', CL_cr, CDi_cr, cd_profile, CD_cr);
fprintf('  L=%.2f lb  Di=%.2f lb  D=%.2f lb  L/D=%.4f\n', L, Di, D, L/D);

%% AoA Sweep
alpha_deg = -4:0.5:14;
CL_s=zeros(1,length(alpha_deg)); CDi_s=CL_s; CD_s=CL_s; LD_s=CL_s;
...
xlabel('\alpha [deg]'); ylabel('Drag Coefficient');
title('Total Drag Coefficient vs. \alpha');
legend('Location','northwest'); grid on;

% Deliverable 5
[maxLD, imax] = max(LD_s);
figure; plot(alpha_deg, LD_s, 'k-', 'LineWidth',2); hold on;
plot(alpha_deg(imax), maxLD, 'r*', 'MarkerSize',12, ...
    'DisplayName', sprintf('(L/D)_{max}=%.2f at %.1f°', maxLD, alpha_deg(imax)));
xlabel('\alpha [deg]'); ylabel('L/D');
title('L/D vs. \alpha'); legend('Location','best'); grid on;
...
%   x_c, y_c    - camber line coordinates (empty [] to skip)
